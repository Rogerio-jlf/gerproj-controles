...
function validarSenha(senha: string): ValidationResult {
    ...
    if (senha.length < 12) {
        warnings.push('Senha tem menos de 12 caracteres - considere usar uma senha mais longa');
    }
    ...
    padroesFracos.forEach((padrao) => {
        if (padrao.test(senha)) {
            warnings.push('Senha contém padrões comuns ou repetições - não recomendado');
        }
    });
    ...
    if (
        ...
        warnings.push('Senha contém sequências alfabéticas - considere variação maior');
    }

    if (/(?:012|123|234|345|456|567|678|789)/.test(senha)) {
        warnings.push('Senha contém sequências numéricas - considere variação maior');
    }
...
// ==================== GERAÇÃO DE HASH ====================
async function gerarHash(senha: string, verbose: boolean = false): Promise<void> {
    console.log('╔═════════════════════════════════════════════╗');
    ...
    if (!validation.valid) {
        ...
        console.error('');
        log(`Tentativa de gerar hash com senha inválida: ${validation.errors.join(', ')}`, 'ERROR');
        process.exit(1);
    }
    ...
    if (verbose) {
        ...
        console.log(`   └─ Letras minúsculas: ${/[a-z]/.test(senha) ? '✅' : '❌'}`);
        console.log(`   └─ Letras MAIÚSCULAS: ${/[A-Z]/.test(senha) ? '✅' : '❌'}`);
...
async function gerarSenhaEHash(comprimento: number): Promise<void> {
    ...
    log(`Senha segura gerada (comprimento: ${comprimento}, força: ${nivel})`, 'INFO');
}
